...
def generate_segment_indices(videopath1, videopath2, num_input_frames=10, filename_tmpl='{:08d}.png'):
    ...
    """
    seq_length = len(glob.glob(f'{videopath1}/*.jpg'))
    seq_length2 = len(glob.glob(f'{videopath2}/*.jpg'))
    ...
    start_frame_idx = 1  # test
    # start_frame_idx = np.random.randint(1, seq_length - num_input_frames)
    # start_frame_idx = np.random.randint(1, 10)
...
class UCF101Dataset(Dataset):
    ...
    def process_dataset(self, ):
        print("Processing ucf-101 dataset ……")

        lines = [line.rstrip() for line in open(self.label_path, 'r')]
        for line in lines:
            idx, label = line.split()
            self.label2idx[label] = int(idx) - 1
            self.idx2label[int(idx)-1] = label

        all_label_names = os.listdir(self.gt_dir)
        for label_name in all_label_names:
            sub_paths = os.listdir(os.path.join(self.gt_dir, label_name))
            self.gt_seq_paths.extend(os.path.join(self.gt_dir, label_name, k) for k in sub_paths)
            self.lq_seq_paths.extend(os.path.join(self.lq_dir, label_name, k) for k in sub_paths)
            self.seq_labels.extend(self.label2idx[label_name] for k in sub_paths)
    ...
    def __getitem__(self, idx):
        ...
        return gt_sequence, lq_sequence, self.seq_labels[idx]
...
if __name__ == "__main__":
    ...
    train_loader = DataLoader(ucf101_dataset, batch_size=1, shuffle=True, num_workers=4, pin_memory=True)

    for gt, lq, label in train_loader:
        print(gt.shape)
        label = torch.LongTensor(label)
        idx = 0
        save_image(gt[0], f'E:/LiuJia/VPSR/{idx}_TEST.png', nrow=1)
        pass