...
# from utils.imgproc import image_resize, expand_y, bgr_to_ycbcr, rgb_to_ycbcr_torch
...
if __name__ == '__main__':
    ...
    get_psnr = PSNR(1, only_test_y_channel=True)
    get_ssim = SSIM(1, True)

    path1 = "E:/LiuJia/VPSR/log_dir/20240420/BasicVSR/HR/"
    path2 = "E:/LiuJia/VPSR/log_dir/20240420/Non_AP_SR/"
    image_names = os.listdir(path1)
    total_psnr, total_ssim = 0.0, 0.0
    for image_name in image_names:
        print(f'Processing {image_name}')
        img1 = Image.open(os.path.join(path1, image_name)).convert('RGB')
        img_tensor1 = torch.tensor(np.array(img1).transpose(2, 0, 1)) / 255. * 2 - 1
        img_tensor1 = img_tensor1.unsqueeze(0).float().to('cuda:0')

        img2 = Image.open(os.path.join(path2, image_name)).convert('RGB')
        img_tensor2 = torch.tensor(np.array(img2).transpose(2, 0, 1)) / 255. * 2 - 1
        img_tensor2 = img_tensor2.unsqueeze(0).float().to('cuda:0')

        psnr_val = get_psnr(img_tensor1, img_tensor2)
        ssim_val = get_ssim(img_tensor1, img_tensor2)
        total_psnr += psnr_val.item()
        total_ssim += ssim_val.item()

    print(f"PSNR:{total_psnr / len(image_names):.4f}, SSIM:{total_ssim / len(image_names):.4f}")
