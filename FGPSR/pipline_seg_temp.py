...
from time_cal import TimeRecorder
from data_util import crop_cpu
...
def combine(sr_list, num_h, num_w, h, w, patch_size=16, step=16, scale=4):
    index = 0
    sr_img = np.full((h * scale + (num_h-1) * 4, w * scale + (num_w - 1) * 4, 3), 255)
    for i in range(num_h):
        for j in range(num_w):
            sr_img[(i * step + i*4):(i * step + patch_size + i * 4),
            (j * step + j*4):(j * step * scale + patch_size + j*4), :] = sr_list[index]
            index += 1
...
def main():
    ...
    for name in tqdm.tqdm(lr_name):
        ...
        tt = combine(lr_list, num_h, num_w, h, w, patchsize, patchsize, scale=1)

        cv2.imwrite('tttttt.png', tt)
