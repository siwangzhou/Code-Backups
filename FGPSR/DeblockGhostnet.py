...
class DeblockGhostNet(nn.Module):
    def __init__(self, pretrained=True):
        super(DeblockGhostNet, self).__init__()
        self.ghostnet = ghost_net()

        self.decoder = nn.Sequential(
            DepthwiseSeparableConv(184, 64),
...
if __name__=='__main__':
    ...
    # model = ghost_net().to('cuda:0')
    model = DeblockGhostNet().cuda()
    model.eval()
    # print(model)

    with torch.no_grad():
        input = torch.randn(1, 3, 480, 312).to('cuda:0')
        for i in range(10):
            TR.start()
            y = model(input)
            TR.end()
