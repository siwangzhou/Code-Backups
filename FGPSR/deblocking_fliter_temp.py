...
for i in range(frame_number):
    filename = f"./data/SR_2.png"
    image = cv2.imread(filename)
    image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    Decompressed[i] = image
    filename = f"./data/GT.png"
    image = cv2.imread(filename)
    image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    Original[i] = image
